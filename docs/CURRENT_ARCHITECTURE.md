...
与当前便携版数据安全直接相关的 `TASK-D-DATA-001`（批量元数据补全安全重构）已经实现：检查 TMDB 可提供的全部字段、只补缺失值、区分电影/剧集/具体季、多个候选要求用户选择，并在 V18 `settings` 中按“记录 ID + IMDb ID + 字段”记忆 TMDB 无数据状态；IMDb ID 变化时自动重新检查。确认写入仍通过正常记录 action。下一项 R0 数据任务是 `TASK-D-DATA-002`（V18/V19 兼容与领域约束），之后是 `TASK-D-DATA-003`（高风险操作自动恢复点）。完整清单和状态以 `.agent-work/TASKS.md` 为准。
