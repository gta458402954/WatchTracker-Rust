...
已完结且从未启用逐集记录的条目只显示“已完成 · 无逐集历史”，不再提供启用入口；Rust 命令同步拒绝这类启用请求，防止绕过界面误改状态。已完结且已有逐集历史的条目显示只读历史摘要，不提供普通“下一集”下拉框。

若已有逐集历史的已完结条目后来增加总集数，并且新总集数大于历史中的最大集号，卡片显示新增集数和“继续追更（第 N 集）”。只有这个入口可以把条目恢复为“在看”，既有完成历史与原完结日期均保留。第一版不提供重看或多轮观看会话；若出现“已看 + 逐集启用 + 下一集非空”的不一致状态，只显示警告而不提供普通操作。
...
本地门禁全部通过：Rust 72/72、Node 70/70、Playwright 58/58，以及 TypeScript typecheck、ESLint、生产构建、rustfmt 和 Clippy。新增验收覆盖显式启用不转换旧进度、跳集三态、完结与后退保留历史、已看未启用记录的界面/命令双层阻断、完结只读展示、新增集续追保留历史与原完结日期、总集数缩小阻断、删除子历史、旧格式导入保留匹配历史、V3→V4 确认升级，以及不同完成时间的本机/云端选择。自动测试仅使用临时 SQLite 与 mock WebDAV，未触碰真实便携数据库或真实远端。
