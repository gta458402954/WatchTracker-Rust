...
| 需要用户处理 | 401/403、无凭据、`conditional_write_unsupported`、`conditional_validator_rejected`、`unsupported_remote_schema`、`legacy_remote_changed` | outbox 保留；停止当前会话自动重试，等待凭据/配置变化或用户手动操作；客户端升级后，历史条件写入错误在下次启动时允许一次安全重试 |
...
2026-08-02 已按本设计完成实施。Rust 在 `settings` 中维护版本化单槽 outbox 与 scheduler；新增、更新、删除、导入替换、冲突选择和恢复点还原均在真实 SQLite 原子边界入队。`commit_sync_result(expectedGeneration)` 只有在 generation 仍匹配时才确认 captured outbox；网络等待期间的新编辑、同步落盘失败或显式旧版冲突导入均不会误清 pending。

前端已由单一协调器统一处理编辑 debounce、启动、窗口聚焦/重新可见、网络恢复、独立周期拉取、暂停/恢复、手动同步和持久退避。暂停跨重启保留但不阻止手动同步；恢复时 pending 立即补跑，否则主动检查。设置页分开展示编辑防抖与默认 15 分钟的主动拉取周期，顶部状态栏区分暂停、待同步和自动同步已开启。

为避免主动拉取造成写放大，纯远端检查在 records/tombstones 没有业务变化时不创建恢复点、不全量替换记录、也不递增 generation，只在同一事务更新同步确认状态。自动化覆盖 outbox 回滚/合并、CAS 确认、恢复点还原、clean pull、跨重载补跑、503 退避和 401 阻断；真实 WebDAV 与正式数据库未用于自动化测试。数据库继续保持 V18，WebDAV 目标隔离仍由 `TASK-D-SYNC-003` 跟踪。

后续 `TASK-D-SYNC-001-R2` 已在 `docs/SYNC_STAGING_DESIGN.md` 增加按记录 ID 的版本暂存、PUT 前发布意图和 SyncCommit 增量落盘，补齐“远端 PUT 成功但本地确认失败”的恢复窗口。单槽 outbox 继续负责调度 generation，版本暂存负责指出具体未确认记录，两者职责不互相替代。
