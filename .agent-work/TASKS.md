...
- Implementation: `syncMerge.ts` 提供不依赖墙上时钟的三方字段合并、删除语义和冲突冻结；`webdav.ts` 使用独立 v3 资源、ETag 条件 PUT、最多三次 412 重拉、commitId 验证、v2 首次迁移及旧客户端变化阻断。普通 GET/PUT 响应缺少 ETag 时，以受限 `PROPFIND Depth: 0` 获取标准 `DAV:getetag`；强 ETag 使用 HTTP `If-Match`，坚果云弱 ETag 使用 RFC 4918 WebDAV `If`，仍无合法 ETag 才阻断上传。Rust 提供稳定设备 UUID、结构化 WebDAV 响应、条件头白名单、同步快照、`expectedGeneration` CAS、恢复点和单事务 SyncCommit/冲突解决。设置页显示具体冲突字段并提供本机/云端或保留/删除选择。
