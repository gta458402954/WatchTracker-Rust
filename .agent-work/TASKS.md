...
- 路线图：19 个领域任务及 1 个同步修订任务；`TASK-D-DATA-001`~`004`、`TASK-D-SYNC-001/002` 与 `TASK-D-SYNC-001-R2` 已实现，剩余为 1 个 `DRAFT`、1 个 `SPECIFIED`、11 个 `NEEDS-DESIGN`。持续集成已移出 DEFERRED，转为维护项。
...
| `TASK-D-SYNC-003` | R0 | 同步隔离 | DRAFT | R0 WebDAV 目标隔离，专项设计待确认 |
...
- Pending Confirmation: 多个 target 代表同一本地影视库的不同同步端点；第一版只允许断开并保留 target，不提供永久删除 target 状态。确认后状态可转为 READY 并进入实施。
