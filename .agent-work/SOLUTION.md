...
> **状态更新（2026-08-02）：** Gate A/B 与路线图 `TASK-D-DATA-001`~`004` 已完成，当前权威源码为本仓库 `main`；正式便携版的精确构建提交号显示在应用顶部栏。本文保留实施过程和目标设计；涉及 Zustand、schema v3/ETag/`expectedGeneration` 同步的内容属于历史候选或后续路线图，不代表当前实现。当前实现边界见 `docs/CURRENT_ARCHITECTURE.md`。
