...
export default function RecordForm({ record, onSave, onDelete, onClose, onNotify, collections = [], collectionMembers = [] }: RecordFormProps) {
    ...
    try {
      ...
      if (isSeason || isEpisode) {
        ...
        const updates: Partial<typeof form> = {
          ...
          tmdbId: targetSeason.id ?? null,
          tmdbParentId: fetchId,
          tmdbSeasonNumber: targetSeason.season_number ?? null,
  ...
  async function handleSelectSeason(season: TmdbSeason) {
    ...
    setForm(prev => ({
      ...
      tmdbParentId: selectedSeries.id ?? null,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          ...
          {collections.length > 0 && <section className="rounded-2xl border border-gray-100 bg-gray-50/70 p-3" aria-labelledby="record-groups-title">
            ...
            <div className="mt-3 flex min-w-0 flex-wrap items-center gap-2">
              {!selectedCollections.length && <span className="text-xs text-gray-400">尚未加入收藏集</span>}
          ...
          {showCollectionManager && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-900/35 p-4" onMouseDown={event => { if (event.target === event.currentTarget) setShowCollectionManager(false); }}>
            <div role="dialog" aria-modal="true" aria-label="管理所属收藏集" className="flex max-h-[75vh] w-full max-w-md flex-col rounded-3xl bg-white p-5 shadow-2xl">
              ...
              <input autoFocus value={collectionSearch} onChange={event => setCollectionSearch(event.target.value)} placeholder="搜索收藏集" className="mt-4 rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-indigo-400" />
              <div className="mt-3 flex-1 space-y-4 overflow-y-auto">
                ...
                {!visibleCollections.length && <p className="py-8 text-center text-sm text-gray-400">没有匹配的收藏集</p>}
              </div>
